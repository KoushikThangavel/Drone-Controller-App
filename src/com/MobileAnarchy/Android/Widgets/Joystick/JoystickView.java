...
////        Decrease moveResolution (e.g., setMoveResolution(0.5f))
////        Increase movementRange (e.g., setMovementRange(20))
////        Decrease Sensitivity:
////        Increase moveResolution (e.g., setMoveResolution(2.0f))
////        Decrease movementRange (e.g., setMovementRange(5))
//
//        setMovementRange(10);
...
//    private Paint handlePaint;
    ...
    private Paint bgPaint;
    private Paint bgSecondaryPaint;
    private Paint handlePaint;
    ...
    //    his is the outer background color of the joystick.
//    private final int BG_COLOR = Color.parseColor("#F5F5F5");
//
//    //    This is the inner background circle color layered on top of the background.
//    private final int SECONDARY_BG_COLOR = Color.parseColor("#FFFFFF");
//    //    private final int HANDLE_COLOR = Color.parseColor("#4285F4");
//
//
////    This is the main color of the joystick's handle/knob.
//
//    private final int HANDLE_COLOR = Color.parseColor("#FFA500");
//
//    //    This is the border around the handle.
//    private final int HANDLE_BORDER_COLOR = Color.parseColor("#FFFFFF");



    // Replace this section of color definitions in your JoystickView.java

    // Outer dark background of the joystick
    private final int BG_COLOR = Color.parseColor("#0D0D0D");

    // Inner circle background (same as outer for deep dark look)
    private final int SECONDARY_BG_COLOR = Color.parseColor("#0D0D0D");

    // Knob color (center handle)
    private final int HANDLE_COLOR = Color.parseColor("#B2662D");

    // Knob border (matching or slightly darker than knob)
    private final int HANDLE_BORDER_COLOR = Color.parseColor("#B2662D");

    // Glow (outer reddish-orange light)
    private final int SHADOW_COLOR = Color.parseColor("#D63D00");




    //    This is the shadow color applied behind the background.
//    private final int SHADOW_COLOR = Color.argb(50, 0, 0, 0);

    // Class variables for gradient caching
    private RadialGradient mHandleGradient;
    private int mLastGradientSize = -1;
    private int mLastCenterX = -1;
    private int mLastCenterY = -1;
    private boolean mGradientNeedsUpdate = true;
    ...
    private void initJoystickView() {
        setFocusable(true);
        setLayerType(LAYER_TYPE_SOFTWARE, null); // Needed for proper shadow rendering

        // Background paint with shadow
        bgPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        bgPaint.setColor(BG_COLOR);
        bgPaint.setStyle(Paint.Style.FILL);
        bgPaint.setShadowLayer(25f, 0f, 5f, SHADOW_COLOR);

        // Secondary background paint
        bgSecondaryPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        bgSecondaryPaint.setColor(SECONDARY_BG_COLOR);
        bgSecondaryPaint.setStyle(Paint.Style.FILL);

        // Handle paint with gradient
        handlePaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        ...
        handleBorderPaint.setColor(HANDLE_BORDER_COLOR);
        handleBorderPaint.setStrokeWidth(6f);
    ...
    // onSizeChanged?
//This is where the size is provided
    @Override
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        super.onMeasure(widthMeasureSpec, heightMeasureSpec);
        // Here we make sure that we have a perfect circle
    ...
    //This is where the position is determined
    @Override
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        super.onLayout(changed, left, top, right, bottom);
    ...
    //This is where the size is calculated
    private int measure(int measureSpec) {
        int result = 0;
        // Decode the measurement specifications.
        int specMode = MeasureSpec.getMode(measureSpec);
    ...
    //This is where the size is set by the user from preferences(settings)
    public void setPreferences(SharedPreferences prefs){
        prefRatio = Float.parseFloat(prefs.getString(PreferencesActivity.KEY_PREF_JOYSTICK_SIZE, "100"));
        prefRatio/=100.0;
        requestLayout();
    ...
    //This is where the JoyStick's UI is painted (Colour and all are calculated and drawn)
    @Override
    protected void onDraw(Canvas canvas) {

        // Update gradient position if needed
        if (handleX != circleCenterX || handleY != circleCenterY) {
            mGradientNeedsUpdate = true;
        }

//        if (mGradientNeedsUpdate) {
//            updateHandleGradient();
//        }
        updateGradient();
...
//        canvas.save();
        ...
        canvas.drawCircle(circleCenterX, circleCenterY, bgRadius * 0.85f, bgSecondaryPaint);
...
//        canvas.restore();
    ...
    private void updateGradient() {
        if (mHandleGradient == null ||
                mLastGradientSize != handleRadius ||
                mLastCenterX != circleCenterX ||
                mLastCenterY != circleCenterY) {

            mHandleGradient = new RadialGradient(
                    circleCenterX,
                    circleCenterY,
                    handleRadius,
                    new int[]{HANDLE_COLOR, adjustAlpha(HANDLE_COLOR, 0.8f)},
                    new float[]{0f, 1f},
                    Shader.TileMode.CLAMP
            );
            handlePaint.setShader(mHandleGradient);
