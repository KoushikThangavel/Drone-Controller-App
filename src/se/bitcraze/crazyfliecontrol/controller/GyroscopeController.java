...
//        private final float AMPLIFICATION = 1.5f;
    ...
    public float getRoll() {
        ...
        // Apply trim once only
        roll += mControls.getRollTrim();

        // Clamp to -1.0 to 1.0 after adding trim
        roll = (float) Math.min(1.0, Math.max(-1.0, roll));

        // Apply sensitivity scaling and deadzone
        return roll * mControls.getRollPitchFactor() * mControls.getDeadzone(roll);
    }
    ...
    public float getPitch() {
        ...
        // Apply trim once only
        pitch += mControls.getPitchTrim();

        // Clamp to -1.0 to 1.0 after adding trim
        pitch = (float) Math.min(1.0, Math.max(-1.0, pitch));

        // Apply sensitivity scaling and deadzone
        return pitch * mControls.getRollPitchFactor() * mControls.getDeadzone(pitch);
    }
