...
import se.bitcraze.crazyfliecontrol2.WalkthroughActivity;
...
//            PreferenceManager.setDefaultValues(getActivity(), R.xml.advanced_preferences, false);
            ...
            PreferenceScreen afcScreen = (PreferenceScreen) findPreference("pref_afc_screen");

            // Disable the entire "Advanced Flight Control" section, including its title
            if (afcScreen != null) {
                afcScreen.setEnabled(false);  // Disables the PreferenceScreen (grays it out)
            }
            ListPreference modePreference = (ListPreference) findPreference(PreferencesActivity.KEY_PREF_MODE);
            if (modePreference != null) {
                // Only include Mode 2
                CharSequence[] entries = {"Left Joystick: Throttle & Yaw and Right: Pitch & Roll"};
                CharSequence[] entryValues = {"2"}; // Match the value in your modeValues array

                modePreference.setEntries(entries);
                modePreference.setEntryValues(entryValues);

                // Optional: force selection if not already set
                SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(getActivity());
                if (!prefs.getString(PreferencesActivity.KEY_PREF_MODE, "").equals("2")) {
                    modePreference.setValue("2");
                }

                modePreference.setSummary(entries[0]);
            }
        ...
        private void setInitialSummaries() {
            ...
            setInitialSummaryAndReturnDefaultValue(KEY_PREF_PITCHTRIM, R.string.preferences_trim_defaultValue_pitch);
            ...
            Preference walkthroughPref = findPreference("pref_walkthrough");
            if (walkthroughPref != null) {
                Log.d("Walkthrough", "Found preference, setting listener.");
                walkthroughPref.setOnPreferenceClickListener(preference -> {
                    Log.d("Walkthrough", "Clicked walkthrough preference.");
                    Intent intent = new Intent(getActivity(), WalkthroughActivity.class);
                    startActivity(intent);
                    return true;
                });
            } else {
                Log.e("Walkthrough", "Preference not found — likely missing from XML.");
            }
