        ...
        mSendJoystickDataThread = new Thread(new Runnable() {
            ...
            public void run() {
                while (mainActivity != null && mCrazyflie != null) {
                    ...
                    float thrustAbsolute = controller.getThrustAbsolute(); // Already scaled -60000 to +60000
                    int mappedThrust = Math.round(thrustAbsolute);  // ✅ Directly use absolute thrust

                    Log.d("THRUST_FINAL", "Thrust Absolute = " + thrustAbsolute);
                    ...
                    if (heightHold) {
                        ...
                        Log.d("THRUST_MAPPED", "Raw: " + thrustAbsolute + ", Mapped: " + mappedThrust);
                        sendPacket(new CommanderPacket(roll, pitch, yaw, mappedThrust, xmode));
                    }
...
//ganesh changes
        ...
        public void logDataReceived(LogConfig logConfig, Map<String, Number> data, int timestamp) {
            ...
            if ("Standard".equals(logConfig.getName())) {
                final float battery = (float) data.get("pm.vbat");
                Log.d("battery", "Battery data received: " + battery);
                mainActivity.setBatteryLevel(battery);

                // 💾 Save to file
                String logLine = "Time: " + timestamp + ", Battery: " + battery + "V\n";

                saveLogToFile(logLine);
            }

            for (Map.Entry<String, Number> entry : data.entrySet()) {
                Log.d("battery", "\t Name: " + entry.getKey() + ", data: " + entry.getValue());
            }
        }


        private void saveLogToFile(String logLine) {
            try {
                File logFile = new File(mainActivity.getCacheDir(), "battery_log.txt");
                Log.d("battery", "Battery log file path: " + logFile.getAbsolutePath());

                FileWriter writer = new FileWriter(logFile, true); // append mode
                writer.append(logLine);
                writer.close();
            } catch (IOException e) {
                Log.d(LOG_TAG, "Error saving log to file", e);
            }
        }
    ...
    private void startLogConfigs(LogConfig logConfig) {
        ...
        //Log.d("Battery", "Adding log listener",+standardLogAdapter);
...
//    private void r() {
//        new Thread(() -> {startTelemetryUdpListene
//            try {
//                DatagramSocket socket = new DatagramSocket(2399); // Listen on this port
//                byte[] buffer = new byte[1024];
//                DatagramPacket packet = new DatagramPacket(buffer, buffer.length);
//
//                TelemetryLogger logger = new TelemetryLogger(mainActivity);
//
//                while (true) {
//                    socket.receive(packet);
//                    String message = new String(packet.getData(), 0, packet.getLength());
//
//                    Log.d("TelemetryUDP", "Received: " + message);
//                    logger.log(message);  // Log to file
//                }
//
//            } catch (IOException e) {
//                Log.e("TelemetryUDP", "Error receiving telemetry", e);
//            }
//        }).start();
//    }
